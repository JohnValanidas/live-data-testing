...
async def get_postgres_listener() -> PostgresListener:
    ...
    if _notify_conn is None:
        _notify_conn = await psycopg.AsyncConnection.connect(settings.pg_dsn.unicode_string(), autocommit=True)
        await _notify_conn.execute(f"LISTEN {settings.notify_channel};")
